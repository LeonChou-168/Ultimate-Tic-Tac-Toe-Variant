...
- The game page’s visual depth issue came from the board frame, sidebar, and shell halo all carrying similarly strong shadow/glass weight. Better hierarchy comes from reducing shell prominence, strengthening the board plane, and slightly softening the sidebar.
- When the target is “same layer” instead of “board first,” the right move is not to keep weakening the sidebar; it is to bring board and sidebar into the same material family and reduce the gap in their shadow/background treatment.
- To make the sidebar feel embedded in the outer shell, the key factors are geometric rather than decorative: reduce the column gap, align the sidebar’s top to the shell inset, and make its silhouette less like an isolated rounded card.
- The referenced prompt’s liquid-glass identity is not just “more blur.” The key ingredients are: a black base, very low-opacity white fills, strong use of backdrop blur, and a masked highlight-border overlay that creates the signature glass edge without normal borders.
- For the board area, the coordinate plane needs to share the same glass shell as the board frame or it still reads like a label accessory. For controls, generic ghost buttons are too flat; targeted glass pills are needed to keep the material system coherent.
- To push the board shell toward a porthole/canopy feel without overworking the coordinate plane, the best leverage is in the outer frame itself: larger radius, stronger top-edge highlight, and a visible inner lip.
- The board shell started to feel over-built once both `board-frame` and `coordinate-frame` behaved like shells. The cleanest simplification is to let `board-frame` remain the only shell and reduce `coordinate-frame` to layout-only structure.
- If the outer shell stops reading as a wrapper after simplification, the answer is not to reintroduce a second shell; it is to restore more breathing room in the outer frame itself.
- If the board still visually exceeds the shell after restoring outer padding, the remaining imbalance is usually the inner footprint, not the shell. Shrinking the board container slightly is safer than continuing to bloat the outer frame.
- The bottom numeric coordinate bar can feel more wrapped than the left axis simply from padding asymmetry. Matching its padding/spacing to the left axis is enough to put them on the same plane without reintroducing extra shell layers.
- Replay surfaces looked out of family once the board shell moved toward a porthole treatment. The clean fix is to give replay the same wrapped-window rim/highlight language instead of leaving it on the older generic card style.
- In this UI, the shared sidebar content selectors (`guide-panel`, `settings-panel`, `tutorial-panel`, `endgame-panel`, `sidebar-menu-panel`) are the real “底页” surfaces. If only replay is upgraded, the sidebar still feels visually inconsistent.
- Once the overall shell is removed, the board region’s hierarchy simplifies: the board frame becomes the outermost shell, the coordinate layout is attached directly to it, and the macro board remains the innermost content layer.
- The persistent “frame smaller than board” bug came from competing width caps on the same element stack (`board-frame` and `board-frame-large`), especially because mobile overrides re-expanded the inner board too aggressively. The fix needs coordinated desktop + breakpoint caps, not one-off padding tweaks.
- The later top-right overflow and entry-time board movement were both consequences of the manual `transform: translate(...)` added to bias the shell toward the upper-right. That transform stacked badly with the game-entry animation and visually pushed the content out of the shell.
- The stable solution was to stop patching mixed responsibilities on one element and instead split the board region into: outer shell, coordinate/layout layer, and actual board content. Once these responsibilities are separated, shell-vs-board sizing becomes predictable again.
- After the split, the first polish lever is no longer structure but proportion: shell thickness, coordinate offset, and macro-board inset depth need to be tuned together or the board will feel either cramped or loosely placed.
- The local `.tsx` reference was not a reusable component but an iframe demo with inline hover transforms. The reusable part was the interaction model: perspective tilt, deeper inset highlight on hover, and a cursor-tracked light hotspot — not the iframe wrapper or literal inline event handlers.
- A shared React parallax state makes all glass components respond together, which feels wrong. The maintainable fix is per-element CSS variable updates written directly to the hovered element, plus a shared physical-motion class in CSS.
- If the outer board shell is itself interactive, hovering any inner board content still makes the frame appear to move. The cleanest interpretation of “外框不要动” is to keep the shell static and reserve physical motion for subordinate glass surfaces only.
- The provided `GlassSurface` prompt is a materially different rendering approach from the existing hand-rolled glass CSS: SVG displacement, `backdrop-filter: url(#filter)`, and fallback styling. A safe migration path is to wrap the major surfaces with `GlassSurface` first and then strip away overlapping legacy glass chrome.
- A partial GlassSurface migration still leaves the UI feeling hybrid. The decisive step is to move the most visible legacy surfaces (landing cards/buttons, sidebar shell, entries, primary panels) onto `GlassSurface` and then delete the unused old motion/state glue.
- The last meaningful cleanup step is removing wrapper-level pseudo-glass ownership (`::before`, inherited fills/blur/shadows, and old motion classes) from classes that now live on `GlassSurface` elements. After that point, the old CSS is mostly just layout and typography, not a competing material system.
